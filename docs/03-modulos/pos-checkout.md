...
3. *Checkout* → el modal unificado `PaymentDialog` (`components/pos/payment-dialog.tsx`): un método, o «Dividir pago» (dos métodos y montos que deben sumar el total de la vista previa). Si hay efectivo, «Recibido» y «Cambio»/«Falta» se calculan en vivo (`cashDifference`) y son solo visuales: no se envían. El mismo diálogo (con `amountEditable` para un abono menor al saldo) cobra una [cuenta abierta](cuentas-abiertas.md) y un abono de [cuentas por cobrar](cuentas-por-cobrar.md); `DeferTabDialog` reutiliza `PaymentFields` para el abono inicial al diferir. `POST /api/v1/sales` lleva ítems `{ product_id, quantity, discount? }` **o** `{ promotion_id, quantity }`, y `payment_method` **o** `payments` (1 o 2).
