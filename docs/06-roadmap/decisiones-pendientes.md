...
| D16 | ¿Se necesita operar **sin conexión**? | Arquitectura del POS | Hoy es imposible (todo va a Supabase). Si sí, requiere cola offline y sincronización — cambio grande | **Parcial: lectura offline implementada** (PWA instalable, vistas visitadas antes se ven sin red, avisos de conexión, instantánea del catálogo del POS para buscar sin red — F1), `create_sale` ya es idempotente (F0, prerequisito de la cola) y cobrar sigue deshabilitado sin red. La cola de escrituras sin red queda **solo documentada como plan**, ver [offline-y-sincronizacion](offline-y-sincronizacion.md) |
