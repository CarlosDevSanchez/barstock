...
| D16 | ¿Se necesita operar **sin conexión**? | Arquitectura del POS | Hoy es imposible (todo va a Supabase). Si sí, requiere cola offline y sincronización — cambio grande | **Parcial: lectura offline implementada** (PWA instalable, vistas visitadas antes se ven sin red, avisos de conexión, instantánea del catálogo del POS para buscar sin red — F1); `create_sale` ya es idempotente (F0) y ya sabe recibir una venta offline con su hora, total provisional y faltante de stock (F2) — pero cobrar sigue deshabilitado sin red porque falta la cola del dispositivo (F3) que la mande. Ver [offline-y-sincronizacion](offline-y-sincronizacion.md) |
