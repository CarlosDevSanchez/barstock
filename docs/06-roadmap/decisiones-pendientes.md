...
| D16 | ¿Se necesita operar **sin conexión**? | Arquitectura del POS | Hoy es imposible (todo va a Supabase). Si sí, requiere cola offline y sincronización — cambio grande | **Implementado para ventas nuevas** (F0–F4): lectura offline (PWA, instantánea del catálogo — F1), `create_sale` idempotente (F0) y listo para una venta offline con su hora/total provisional/faltante de stock (F2), **cobrar funciona sin red** — se encola en IndexedDB y un motor de sincronización lo manda solo al volver la conexión (F3) — y la UI de seguimiento está construida: centro de sincronización con contador/retry/discard, ticket "PROVISIONAL" y revisión de gerente para `sync_issues` (F4). Cuentas abiertas, ajustes de inventario y reembolsos siguen deshabilitados sin red (fuera de alcance v1, decisión sin revisar). Ver [offline-y-sincronizacion](offline-y-sincronizacion.md) |
