...
| `outbox/discard-log` | POST | gerente | RPC `log_outbox_discard`; `{ provisional_number, expected_total, payment_method, reason }`; registra en `audit_log` el descarte de una venta offline en cola que nunca llegó al servidor ([offline y sincronización](../06-roadmap/offline-y-sincronizacion.md) §9.2) |
