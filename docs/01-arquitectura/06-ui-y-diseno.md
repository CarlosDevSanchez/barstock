...
| `MoneyInput` (`components/money-input.tsx`) | Campo de dinero controlado (`lib/money-input.ts`): formatea con los separadores del locale al perder el foco y parsea cada tecla mientras se escribe. Lo usan `PaymentDialog` (carrito, detalle de cuenta, abono de `/receivables`) y `PaymentFields` dentro de `DeferTabDialog` |
