...
> **Estado: implementado** (etapa 1, Pasos 3–4; migraciones `…02`–`…05`). Confianza: **[Verificado]** con SQL y contra la API real.
...
- Por línea: `base = precio × cantidad − descuento de línea`; `impuesto = round(base × tax_rate, 2)`. Total de la orden =
...
- Rechaza: carrito vacío (o > 200 líneas), cantidad ≤ 0, descuentos negativos o con más de 2 decimales, cliente inactivo, producto
...
Solo cuentan órdenes `completed` (los reembolsos se excluyen). Los días se agrupan en la zona horaria de `settings.timezone` (por defecto `UTC`).
