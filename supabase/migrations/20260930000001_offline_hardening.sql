-- 0020 offline hardening: fixes from an adversarial review of F0-F4 (offline POS sales).
--   1. Idempotency hash no longer includes occurred_at/expected_total, and the idempotency check now runs before
--      any state-dependent validation (customer/product availability). Without this, a sale that committed online
--      but whose response was lost could get resent through the offline queue with the same client_ref and a
--      different hash (because it now carries occurred_at/expected_total), come back as a 409, and get retried by
--      hand -> the same sale charged twice.
--   2. A customer deactivated, or a product/promotion deactivated, while the till was offline no longer rejects an
--      offline sale outright (money already changed hands): it is recorded with the customer cleared or the last
--      known price used, and flagged in sync_issues for a manager. Products/promotions are soft-delete only, so a
--      row (and its last price) always exists to fall back to.
--   3. A discount that no longer fits (price dropped while offline) is clamped instead of rejecting the sale.
--   4. Every offline-sourced order is now flagged for manager review (sync_issues.offline_sale), even with nothing
--      else off: occurred_at/expected_total are client-supplied and reachable through the plain online API too, so
--      this is the only way to guarantee a human eventually looks at every sale that used the offline leniency.
--   5. order_items gains stock_taken: refund_order now reponies exactly what was deducted, not the full line
--      quantity - an offline sale recorded with a stock shortfall was refunding back stock that was never taken.
--   6. A new log_outbox_discard RPC lets a manager's decision to discard a queued (never-synced) offline sale leave
--      an audit trail, instead of vanishing from IndexedDB with no record anywhere.

-- ---------------------------------------------------------------------------
-- order_items.stock_taken: how much inventory this line actually deducted (may be less than quantity for an
-- offline sale recorded with a stock shortfall). Null on rows written before this migration: refund_order falls
-- back to quantity for those, same as before.
-- ---------------------------------------------------------------------------
alter table public.order_items add column stock_taken int;

-- ---------------------------------------------------------------------------
-- create_sale: idempotency ordering/hash fix, offline leniency for customer/product/promotion/discount, mandatory
-- review flag for every offline order. Same signature as 20260928000001_offline_sales.sql: no drop needed.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if p_customer_id is not null
    ...
    if v_source = 'offline' then
      ...
      -- record it walk-in and flag it for a manager instead (F2's "never reject" principle).
      v_customer_id := null;
      v_sync_issues := v_sync_issues || jsonb_build_object('customer_unavailable', true);
    else
  ...
  loop
    ...
    if v_has_promo then
      ...
      if not v_pr_ok then
        v_sync_issues := v_sync_issues || jsonb_build_object('stale_pricing', true);
      end if;
      ...
      if v_source = 'offline' and exists (
        select 1
        from public.promotion_items pi2
        join public.products p2 on p2.id = pi2.product_id
        where pi2.promotion_id = v_promo_id and not (p2.is_active and p2.deleted_at is null)
      ) then
        v_sync_issues := v_sync_issues || jsonb_build_object('stale_pricing', true);
      end if;
  ...
  loop
    ...
    if v_item.assigned then
      ...
      -- name/existence is looked up leniently too (its promotion was already flagged above if stale).
...
-- ---------------------------------------------------------------------------
-- refund_order: repay exactly what stock_taken deducted, not the full line quantity (an offline sale with a
-- recorded stock shortfall only ever took what was on hand).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- audit_log: allow a 'discard' action, and a log_outbox_discard RPC so discarding a queued (never-synced) offline
-- sale from the sync center leaves an audit trail instead of vanishing from IndexedDB with no record anywhere.
-- ---------------------------------------------------------------------------
alter table public.audit_log drop constraint audit_log_action_check;
alter table public.audit_log add constraint audit_log_action_check check (action in
  ('insert', 'update', 'delete', 'login', 'login_failed', 'logout', 'invite', 'password_reset', 'discard'));

create or replace function public.log_outbox_discard(
  p_provisional_number text,
...
declare
  ...
  v_reason text := nullif(btrim(p_reason), '');
begin
  -- Discarding a queued sale forgoes money already collected offline: gated to manager+ like refund_order, and
  -- always logged as the caller's own JWT, never a parameter.
  if v_uid is null or not public.has_min_role('manager') then
    raise exception 'Not allowed' using errcode = '42501';
  end if;
  if v_reason is null or length(v_reason) < 3 then
  ...
  values (
    v_uid, v_email, v_role, 'discard', 'outbox', p_provisional_number,
    jsonb_build_object(
      'provisional_number', p_provisional_number,
...
$$;
revoke all on function public.log_outbox_discard(text, numeric, text, text) from public, anon;
grant execute on function public.log_outbox_discard(text, numeric, text, text) to authenticated;
