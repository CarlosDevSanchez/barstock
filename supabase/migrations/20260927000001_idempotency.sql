-- 0017 idempotency: protects create_sale against a duplicate charge when the client never sees the response
-- (network drop after COMMIT, or the cashier pressing "Cobrar" again before the first request returns). This is
-- F0 of docs/06-roadmap/offline-y-sincronizacion.md (a prerequisite for offline sales, but fixes a bug that exists
-- today with or without offline support).
--
-- Idempotency is tracked per RPC call (one row per `key`), not per resulting row: a single create_sale call can
-- insert several order_items/inventory_transactions rows (promotion packages expand into multiple lines), so a
-- unique column on those tables cannot represent "this call already happened".
...
create table public.idempotency_keys (
  ...
  -- Null while the owning call is still running; that same call fills it in before it returns, in the same
  -- transaction as the row it describes. A committed row can therefore never be left with a null result.
  result        jsonb,
...
-- No policies: only SECURITY DEFINER RPCs touch this table (they run as the function owner and bypass RLS).
-- The revoke below is belt-and-suspenders in case a future grant is added at the schema level.
revoke all on public.idempotency_keys from anon, authenticated;

-- ---------------------------------------------------------------------------
-- create_sale: add p_idempotency_key
-- ---------------------------------------------------------------------------
-- A new parameter with a default does not replace the old signature, it overloads it (the old grants would still
-- stand). Drop the old signature explicitly so only the new one exists.
drop function public.create_sale(uuid, jsonb, public.payment_method, numeric);

-- Error convention (see 0004's header): P0001/P0002/42501 as before. `BS409` is this project's own application
-- code for "conflict raised from PL/pgSQL, not a real unique-constraint violation" — mapped to HTTP 409 in
-- lib/server/errors.ts, the same way P0001 is mapped to 422.
...
revoke all on function public.create_sale(uuid, jsonb, public.payment_method, numeric, uuid) from public, anon;
grant execute on function public.create_sale(uuid, jsonb, public.payment_method, numeric, uuid) to authenticated;
