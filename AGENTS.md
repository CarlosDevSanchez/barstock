...
| Órdenes de compra, variantes | Las compras recibidas ya tienen API/UI (`receive_purchase`); las variantes de producto siguen solo en esquema. Los gastos operativos y las cuentas por cobrar también tienen API y pantalla | [proveedores](docs/03-modulos/proveedores-y-compras.md), [gastos](docs/03-modulos/gastos.md), [cuentas por cobrar](docs/03-modulos/cuentas-por-cobrar.md) |
...
| Cuentas abiertas (`tabs`) | El stock baja **al añadir** el producto a la cuenta, no al cerrarla; quitar un ítem (gerente+) o anular la repone. Anular solo funciona **sin pagos**. Diferir a cuenta por cobrar no exige cliente (basta un nombre libre) | [cuentas-abiertas](docs/03-modulos/cuentas-abiertas.md), [cuentas-por-cobrar](docs/03-modulos/cuentas-por-cobrar.md) |
| Jornada y cajas | Se puede vender sin jornada ni caja: esas órdenes quedan con ambos ids nulos. El efectivo esperado resta los gastos en efectivo de esa caja. Tolerancia de arqueo 0. Un cron diario cierra jornadas de más de 24 h. **Arqueo a ciegas** (R-1): con la caja abierta, `cash_session_summary` solo devuelve `opening_float` a un cajero (ni siquiera al dueño de la caja) — nada de `expected_cash`, `cash_sales`, `deposits`, etc., porque sumados dan el mismo esperado; se ve todo al cerrar (`close_cash_session` lo devuelve) o si eres gerente+. `getCashDesk` (`/cash`, `/expenses`, `/inventory`) solo llama esa RPC para la caja del propio cajero; para las cajas de otros cajeros en la misma jornada no la llama (lanzaría 42501). Un reembolso en efectivo se resta de la caja **original del pago** (`payments.cash_session_id`) si sigue abierta; si ya cerró, no se resta de ninguna y la orden queda `refund_after_close = true`. Anular un gasto/compra en efectivo con la caja ya cerrada no cambia el esperado (`voided_after_close`) | [caja-y-jornada](docs/03-modulos/caja-y-jornada.md), [H6](docs/04-auditoria/hallazgos/H6-revision-adversarial-a-f.md) |
| Notificaciones (outbox) | `_claim_outbox` reclama filas con `claimed_at` de más de 10 min e incrementa `attempts` al reclamar (antes solo lo hacía al fallar, así que una fila que solo se reclamaba y nunca fallaba podía reclamarse cada 10 min para siempre); `_enqueue_due_receivables` no encola un segundo recordatorio si el de un día anterior sigue sin procesar. `afterResponse` debe recibir la promesa de la tarea directamente (`() => dispatchOutbox()`), nunca envuelta en `void`/llaves, o Vercel puede matar la función antes de que termine. `deletePushSubscriptionSchema` no aplica la allowlist de hosts del push (borrar debe funcionar aunque el host ya no esté soportado). Reintento por destinatario, no por lote: cada fila guarda `payload.delivered_to` y un reintento solo alcanza a quien falta (`_mark_outbox_delivery`); sin ningún canal (correo/push) configurado, `dispatchOutbox` ni siquiera reclama filas. Correo y push son grupos de variables **independientes** (`lib/env/schema.ts`), no uno solo de cinco. Equipo compartido: `register_push_subscription` reasigna `user_id` por `endpoint` (evita el 409 de un insert directo); el logout desuscribe el push del navegador antes de cerrar sesión; el interruptor de push en el menú de cuenta refleja si ESTE navegador tiene una suscripción real, no solo la preferencia guardada. `_purge_outbox()` (cron diario) borra filas procesadas de hace más de 30 días | [H6](docs/04-auditoria/hallazgos/H6-revision-adversarial-a-f.md), [notificaciones](docs/03-modulos/notificaciones.md) |
...
| `mock.module` (Bun) | Es **global al proceso** y se filtra entre archivos: unitarias / componentes (un proceso por archivo) / integración van separadas; la suite sin `indexedDB` vive en `test/offline/` por la misma razón | [testing](docs/05-guias/testing.md) |
...
| PWA / offline | Lectura offline (vistas ya visitadas + instantánea del catálogo del POS, F1). **Cobrar ya funciona sin red** (F3): encola la venta en IndexedDB (`lib/offline/outbox.ts`) y un motor de sincronización (`hooks/use-outbox-sync.ts`, montado en `AppShell`) la manda sola al volver la conexión — es la única escritura habilitada sin red; cuentas abiertas, ajustes de inventario y reembolsos siguen deshabilitados (`OfflineDisabledButton`). El service worker solo se registra en `NODE_ENV=production` (`next dev` lo rompería con Turbopack). `clients.claim()` dispara `controllerchange` también en la primera instalación, no solo en una actualización real: `sw-register.tsx` solo recarga si YA había un controlador previo | [pwa-offline](docs/01-arquitectura/09-pwa-offline.md), [plan de cola](docs/06-roadmap/offline-y-sincronizacion.md) |
